  ...
  TEmployeePostgresRepository =
    class (TAbstractPostgresRepository, IEmployeeRepository)
      ...
      private
        ...
        FEmployeeTableSchema: TEmployeeTableSchema;
      ...
      public
        ...
        constructor Create(
          EmployeeTableSchema: TEmployeeTableSchema;
...
function TEmployeePostgresRepository
  .CheckThatSingleRecordSelected(DataReader: IDataReader): Boolean;
begin
...
constructor TEmployeePostgresRepository.Create(
  EmployeeTableSchema: TEmployeeTableSchema; QueryExecutor: IQueryExecutor);
begin

  inherited Create(QueryExecutor);

  FEmployeeTableSchema := EmployeeTableSchema;
...
begin
  ...
  with FEmployeeTableSchema.EmailTableDef do begin

    EmployeeContactInfoMappings.SetTableNameMapping(
      TableName, TEmployeeContactInfo
    );
...
begin
  ...
  with FEmployeeTableSchema.EmployeeTableDef do begin

    EmployeeMappings.SetTableNameMappingWithAlias(
      TableName, TEmployee, 'emp1', TEmployees
    );

    EmployeeMappings.AddColumnMappingForSelect(IdColumnName, TEmployee.IdentityPropName);
    EmployeeMappings.AddColumnMappingForSelect(LegacyIdColumnName, TEmployee.LegacyIdentityPropName);
    EmployeeMappings.AddColumnMappingForSelect(NameColumnName, TEmployee.NamePropName);
    EmployeeMappings.AddColumnMappingForSelect(SurnameColumnName, TEmployee.SurnamePropName);
    EmployeeMappings.AddColumnMappingForSelect(PatronymicColumnName, TEmployee.PatronymicPropName);
    EmployeeMappings.AddColumnMappingForSelect(SpecialityColumnName, TEmployee.SpecialityPropName);
    EmployeeMappings.AddColumnMappingForSelect(PersonnelNumberColumnName, TEmployee.PersonnelNumberPropName);
    EmployeeMappings.AddColumnMappingForSelect(DepartmentIdColumnName, TEmployee.DepartmentIdentityPropName);
    EmployeeMappings.AddColumnNameForSelect(TopLevelEmployeeIdColumnName);
    EmployeeMappings.AddColumnMappingForSelect(IsForeignColumnName, TEmployee.IsForeignPropName);
    EmployeeMappings.AddColumnMappingForSelect(IsDismissedColumnName, TEmployee.IsDismissedPropName);
    ...
    EmployeeMappings.AddColumnMappingForModification(EMPLOYEE_TABLE_LEADER_ID_FIELD, 'TopLevelEmployee.Identity');
...
begin
  ...
  FreeAndNil(FEmployeeTableSchema);
...
begin
  ...
  { refactor: replace remove methods calls to update for aggregate method calls }
  ...
  { throw if data manipulation error }
  ...
  Result :=
    Format(
      ...
      [
        ...
        EmployeeRoleIdColumnName,
        CreateSQLValueListString(FEmployeeRoleIds),
