...
begin
  ...
  try
    ...
    if Assigned(TopsOfLayoutItemElements) then
      SetLayoutItemTops(TopsOfLayoutItemElements);
...
begin
  ...
  do begin
    ...
    if LayoutItem is TVerticalBoxLayoutManager then begin
      ...
      VerticalBoxLayoutManager := TVerticalBoxLayoutManager(LayoutItem);

      LayoutManagerItem :=
        VerticalBoxLayoutManager[FCurrentElementsIndexOfLayoutItems];

      if Assigned(LayoutManagerItem) then begin
...
procedure TColumnarCellAlignedLayoutManager.SetLayoutItemTops(
  TopsOfLayoutItemElements: TVariantList
);
