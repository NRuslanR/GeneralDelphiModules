  ...
  TAbstractDatabaseMessagingService =
    class (TInterfacedObject, IMessagingService)
      ...
      private
        ...
        FPreparedMessagesDataInsertingQueryPattern: String;
      ...
      protected
        ...
        function PrepareMessagesDataInsertingQueryPattern(
          SchemaData: TDatabaseMessagingServiceSchemaData
        ): String; virtual;
      ...
      protected
        ...
        procedure ExecuteMessagesDataInsertingQuery(
...
begin
  ...
  FPreparedMessagesDataInsertingQueryPattern :=
    PrepareMessagesDataInsertingQueryPattern(SchemaData);
...
procedure TAbstractDatabaseMessagingService.ExecuteMessagesDataInsertingQuery(
  const QueryPattern: String;
  Messages: IMessages
);
var MessagePropertyValueListString: String;
    Values: String;
    QueryText: String;
...
begin
  ...
  for Message in Messages do begin
    ...
    MessagePropertyValueListString :=
      '(' +
      AsSQLString(MapMessageNameDataFrom(Message.Name)) + ',' +
      AsSQLString(MapMessageContentDataFrom(Message.Content)) + ',' +
      AsSQLString(MapMessageSenderDataFrom(Message.Sender)) + ',' +
      AsSQLString(MapMessageReceiversDataFrom(Message.Receivers)) + ',' +
      AsSQLString(MapMessageAttachmentsDataFrom(Message.Attachments)) +
      ')';

    if Values = '' then
      Values := MessagePropertyValueListString

    else Values := Values + ',' + MessagePropertyValueListString;
    
  end;

  QueryText := QueryPattern + ' ' + Values;
...
begin
  ...
  if not (Message.Self is TCommonMessage) then  begin

    raise TAbstractDatabaseMessagingServiceException.Create(
      'Ќе известен тип отправл€емого сообщени€ ' +
      'дл€ выполнени€ разбора'
    );
...
function TAbstractDatabaseMessagingService.PrepareMessagesDataInsertingQueryPattern(
  SchemaData: TDatabaseMessagingServiceSchemaData): String;
begin
