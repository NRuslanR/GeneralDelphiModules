...
begin
  ...
  ChangedQuery :=
    ReplaceStr(
      ...
      EscapeString(VarToStr(QueryParams[SchemaData.MessageContentQueryParamName].ParamValue))
  );
...
begin
  ...
  Result :=
    Format(
      'SELECT sys.sendemail_queue_add(:%s,:%s,:%s,CAST(now() AS timestamp without time zone),:%s)',
      [
